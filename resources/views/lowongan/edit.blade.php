			  	...
			  	<form action="{{ route('lowongan.update',$low->id) }}" method="post" >
					...
					<div class="form-group {{ $errors->has('tgl_mulai') ? ' has-error' : '' }}">
			  		...
			  		<div class="form-group {{ $errors->has('deskripsi_iklan') ? ' has-error' : '' }}">
						...
						<textarea name="deskripsi_iklan" class="form-control" rows = "10"   required>{{ $low->deskripsi_iklan }}</textarea>
						...
						<textarea name="status" class="form-control" rows = "10"   required>{{ $low->status }}</textarea>
